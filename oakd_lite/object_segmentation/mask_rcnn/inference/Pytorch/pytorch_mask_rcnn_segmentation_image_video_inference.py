...
def filter_detections( outputs, dataset_class_names, detection_threshold=0.8):
...
def draw_boxes( boxes, unint8_tensor, pred_classes, labels, colors, fill=False,is_instance=False):
    if is_instance:
    ...
    result_with_boxes = draw_bounding_boxes(image=unint8_tensor, boxes=torch.tensor(boxes), width=2, colors=plot_colors,labels=pred_classes,fill=fill,font="arial.ttf",font_size=40)
    return result_with_boxes, plot_colors, no_detection_result

def draw_instance_mask( outputs, uint8_tensor, colors, no_detection_result, detection_threshold=0.8):
    # Get all the scores.
    ...
    seg_result = draw_segmentation_masks(uint8_tensor, final_masks,colors=colors,alpha=0.8)
...
def main():
    ...
    parser.add_argument('--weight', type=str, help='saved model')
    ...
    for name in os.listdir(args.in_path):
        print(args.in_path+"/"+name)
        if name.endswith('.jpg'):
            s_time = time.time()
            rgb_image = cv2.imread(args.in_path+"/"+name)
            #print(args.in_path+"/"+name)
            rgb_image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
            #rgb_image = T.Compose(T.ToTensor(rgb_image))
            uint8_frame, float32_frame = get_transformed_image(rgb_image)
            float32_frame = torch.squeeze(float32_frame)
            with torch.no_grad():
                prediction = model([float32_frame.to(device)])
            ...
            boxes, pred_classes, labels, pred_scores = filter_detections(prediction, class_names, confident_level)
            print(pred_scores)
            print(pred_classes)
            # Draw boxes and show current frame on screen.
            result, plot_colors, no_detection_result = draw_boxes(boxes, uint8_frame, pred_classes, labels, colors, is_instance=True)
            # Draw the segmentation map.
            result,final_masks = draw_instance_mask(prediction, result, plot_colors, no_detection_result, confident_level)
            ...
            mix = cv2.addWeighted(float32_frame,0.8,mask_3_channel,1.0,0)
